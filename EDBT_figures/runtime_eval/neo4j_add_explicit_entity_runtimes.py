"""
Attempt at a first template script to evaluate runtimes for the explicit_entity model in Neo4j.
"""
...
if __name__ == "__main__":
    print("Evaluation for Explicit Entity Model")
    fn = "./entities.json"
    with open(fn) as f:
        data = json.load(f)
    ...
    for window in [0, 1, 2, 5, 10, 20]:
        ...
        subprocess.run(["ln", "-s", "/data/daumiller/hypergraphs-bp/explicit_entity_model/graph_{}.db".format(window),
        ...
        for iteration in range(6):
            ...
            for i, (entity_label, properties) in enumerate(data.items()):
                ...
                query = "PROFILE MATCH (t:Term {{term_text: '{}'}})-[:T_IN_E]-(hyperedge)- [m:T_IN_E]-(term) " \
                ...
                if iteration > 0:  # take one round of cache warm-up
                    times = data[entity_label].get("explicit_entity_neo4j", {})

                    window_times = times.get(str(window), [])
                    window_times.append(time_taken)
                    times[str(window)] = window_times
                    data[entity_label]["explicit_entity_neo4j"] = times
